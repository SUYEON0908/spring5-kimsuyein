...
- jsp파일-자바프로그램 태그 <% %>
- ex) PHP <?php ?>
- @ page골뱅이는 애노테이션으로 발음-페이지지시자는 필수 랭귀지:java, 
- 콘텐츠타입:text/html; 문자셋:유니코드  페이지인코딩:UTF-8 
자바주석 끝 --%>

<!-- HomeController.java 에서 index.jsp로 매칭 시킨 결과가 나오는 페이지<br>
- HTML에서는 /resoureces/sample/index.html<br>
- jsp프로그램에서는 위 html처럼 바로 접근 못하고, 홈 컨트롤러자바를 거쳐서 index.jsp를 접근하는 이유는 보안<br>
- 은행, 관공서, 대학에서 대부분 사용(스프링을 배우는 목적)<br>
- 다른 예로, php프로그램은 위에 있는 html처럼 파일에 바로 접근이 가능(그래서, 중소기업홈페이지에서 사용)<br>
- 출발은 자바(pc용애플리케이션,웹프로그램도 만드는데 모두 사용)<br>
...
- jsp?: 지금 보이는 프로젝트는 스프링MVC프로젝트<br>
- 여기서 MVC에서 View단에 해당하는 기술 부분이 jsp<br>
